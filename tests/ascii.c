...
int main(void)
{
    ...
    TEST(number_is_numeric, {
        ...
        for (int i = 0; i < strlen(numbers); i++)
            ASSERT_FN(ac_ascii_is_numeric, numbers[i], "'%c'");
    });
    ...
    TEST(nonnumber_isnt_numeric, {
        ...
        for (int i = 0; i < strlen(non_numbers); i++)
            ASSERT_NFN(ac_ascii_is_numeric, non_numbers[i], "'%c'");
    });
    ...
    TEST(alphanumeric_when_alphanumeric, {
        for (int ch = 0; ch < 127; ch++)
            if (ac_ascii_is_alphabetic(ch) || ac_ascii_is_numeric(ch))
            {
                ASSERT_FN(ac_ascii_is_alphanumeric, ch, "'%c'");
            }
            else
            {
                ASSERT_NFN(ac_ascii_is_alphanumeric, ch, "'%c'");
            }
    });
    ...
    TEST(whitespace_is_whitespace, {
        ...
        for (int i = 0; i < strlen(chs); i++)
            ASSERT_FN(ac_ascii_is_whitespace, chs[i], "'%c'");
    });
    ...
    TEST(lower_is_lowercase, {
        ...
        for (int i = 0; i < strlen(chs); i++)
            ASSERT_FN(ac_ascii_is_lowercase, chs[i], "'%c'");
    });

    TEST(lower_isnt_uppercase, {
        char* chs = "abcdefghijklmnopqrstuvwxyz";
        for (int i = 0; i < strlen(chs); i++)
            ASSERT_NFN(ac_ascii_is_uppercase, chs[i], "'%c'");
    });

    TEST(upper_is_uppercase, {
        char* chs = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        for (int i = 0; i < strlen(chs); i++)
            ASSERT_FN(ac_ascii_is_uppercase, chs[i], "'%c'");
    });

    TEST(upper_isnt_lowercase, {
        char* chs = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        for (int i = 0; i < strlen(chs); i++)
            ASSERT_NFN(ac_ascii_is_lowercase, chs[i], "'%c'");
    });
    ...
    TEST(convert_to_other_case, {
        ...
        {
            ASSERT_EQ(upper[i], ac_ascii_to_uppercase(lower[i]), "%c");
            ASSERT_EQ(lower[i], ac_ascii_to_lowercase(upper[i]), "%c");
        }
    });
    ...
    TEST(convert_to_same_case, {
        ...
        {
            ASSERT_EQ(lower[i], ac_ascii_to_lowercase(lower[i]), "%c");
            ASSERT_EQ(upper[i], ac_ascii_to_uppercase(upper[i]), "%c");
        }
    });
    ...
    TEST(nonalphabet_convert_case, {
        for (int ch = 0; ch < 127; ch++)
        {
            ...
            {
                ASSERT_EQ(ch, ac_ascii_to_uppercase(ch), "%c");
            }
        }
    });
